...
#include<stdlib.h>
#include<string.h>
#include<conio.h>//换色系统
#include<windows.h>//sleep函数
void Colorsetting();//选颜色
void menu();
void sign();
void process();
void sys_menu();
...
typedef struct NAME//用户名系统
{
	char name[50];
	char pwd[50];//password
}Name;
 nba *savei(nba *head)//保存系统
{
	FILE *fp;
	nba *p;
	char filmname[40];
	...
	printf("\n\t输入要保存的文件名：  ");
	scanf("%s", filmname);//取一个文件名
	if ((fp = fopen(filmname, "a+")) == NULL)//防止没文件出错
	{
		printf("\t写文件出错，按任意键退出！");
		getch();//吃
		exit(1);//1表示文件问题
	}
	for (p = head->next; p != NULL; p = p->next)//实现保存
	{
		fprintf(fp, "%s %d %s %.1f %.1f %.1f\n", p->name, p->num, p->team, p->score, p->backboard, p->assist);
	}
	fclose(fp);
	printf("\n\t文件已经保存成功！");
	getch();//吃
...
void process()
{
	system("cls");
	printf("\n\n\n\n\n\n\n");
	printf("\t\t恭喜登陆成功！即将进入NBA的世界，系好安全带，出发\n");
	Sleep(1100);
	system("cls");
	printf("\n\n\n\n\n\n\n");
	printf("\t\t史蒂芬库里超远三分球出手 ...\n");
...
nba *del(nba *head)//传个头指针
{
	...
	while (q)
	{
		if (strcmp(q->name, id) == 0)//判定文件已存入某球员名，如果两者相等则执行
		{
			p->next = q->next;
			free(q);
			q = p->next;//后一个指针放到前一个
...
nba *rank2(nba *head)
{
	...
	while (t != head->next)
	{
		...
		while (q->next != t)
		{
			if (p->next->backboard<q->next->backboard)
			{
				p->next = q->next;
				q->next = q->next->next;
				p->next->next = q;
