...
//	//int sum = (*pAdd)(1, 2);
//	//int sum = pAdd(1, 2);也可以 *没用
//			//Add(1, 2);也可以
//	//printf("sum = %d\n", sum);
//	//函数指针的数组
//	int(*pArr[5])(int, int);
//	//指向函数指针数组的指针ppArr=&pArr
//	int(*(*ppArr)[5])(int, int) = &pArr;
//	
//	return 0;
//}
//void bubble_sort(int arr[],int sz)//两两相邻的数交换
//{
//	int i = 0, j = 0;
//	for (i = 0; i < sz - 1; i++)//i 趟
//	{
//		//某一趟排序
//			for (j = 0; j < sz-1-i;j++)
//			if (arr[j]>arr[j + 1])//变的是比较方法
//			{
//				int tmp = arr[j];
//				arr[j] = arr[j + 1];
//				arr[j + 1] = tmp;
...
//int cmp_int(const void* e1,const void *e2)//防止类型混淆，用void
//{
//	return  *(int *)e1 - *(int *)e2;//比较两个整型值, 强制转换int*  再解引用来比较
//}
//
//
//void test1()
...
//	qsort(arr, sz, sizeof(arr[0]),cmp_int);//该库函数使用方法：变量名，大小，字节长度,函数
//	int i = 0;
//	for (i = 0; i < sz; i++)
//	{
//		printf("%d\n", arr[i]);
//	}
//}
//int cmp_float(const void* e1, const void *e2)//防止类型混淆，用void
//{//函数返回类型是int， 但是return了float 因此要改成循环
//	/*if (*(float *)e1 == *(float *)e2)
//	return  0;
//	else if ((*(float *)e1 > *(float *)e2))
//	return 1;
//	else return -1;*/
//	return ((int)*(float *)e2 - *(float *)e2);//也可以强制再换
//}
//void test2()
//{
...
//	qsort(s, sz, sizeof(s[0]), cmp_stu_by_age);//结构体比较是复杂的
//	int i = 0,j=0;
//	for (i = 0; i < sz; i++)
//	{
//		printf("%d\n", s[i].age);
...
//	int sz = sizeof(arr) / sizeof(arr[0]);
//	//使用冒泡的程序员一定知道自己排序的是什么数据
//	//就应该知道如何排序比较数组中的元素  那就要一个函数
...
//}
//int main()//qsort函数演示回调函数
//{
//
//
//
//	//float f[] = { 9.0, 8.0, 7.0, 6.0, 5.0,4.0 };
//	//bubble_sort(arr, sz);//只能一种类型不通用
//	/*test1();
//	test2();
//	test3();*/
//	test4();
